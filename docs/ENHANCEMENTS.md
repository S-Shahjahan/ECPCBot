...
- Search is isolated to approved sources for the selected client. Libraries up to 50,000 extracted characters are supplied in full alongside direct facts. Larger libraries use up to ten ranked passages searched across all approved source chunks and worksheets; no worksheet is excluded merely because it is not first. The catalogue quotation path scans all approved pricing sources, independently of the ten-passage retrieval limit. This keeps provider context bounded but cannot guarantee that every useful passage is selected. For a large multilingual catalogue or semantic matching, evaluate a multilingual embedding model and vector retrieval later. A separate embedding API is not needed for this release.
